...
export async function devices(_values: CliOptions): Promise<number> {
  ...
  for (const device of all) {
    ...
    const name =
      [device.manufacturerName, device.productName].filter(Boolean).join(' ') || 'unknown';
    const configuration = device.configuration ?? device.configurations[0];
    const printer = configuration?.interfaces.find(
      (i) => i.alternate.interfaceClass === PRINTER_CLASS,
    );

    if (printer) {
      printers++;
      const out = printer.alternate.endpoints.find(
      ...
      console.log(
        `    ${dim(`--vendor 0x${hex(device.vendorId)} --product 0x${hex(device.productId)}`)}`,
      );
    } else {
